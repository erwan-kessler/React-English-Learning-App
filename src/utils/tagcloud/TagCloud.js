...
const handlersPropNames = ['onClick', 'onDoubleClick', 'onMouseMove','onPress'];
...
function renderTags(props, data) {
  const { minSize, maxSize,handler } = props;
  ...
  return data.map(({ tag, color }) => {
    ...
    const elem = props.renderer(tag, fontSize, color,handler);
...
export function TagCloud(props) {
  ...
  return <View {...other}>{renderTags(props, data)}</View>
}
